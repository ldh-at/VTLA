...
from ..tactile import TactileTokenEncoder
...
class PI0Pytorch(nn.Module):  # see openpi `PI0Pytorch`
    ...
    def __init__(self, config: PI0Config, rtc_processor: RTCProcessor | None = None):
        ...
        self.tactile_feature_names = list(config.tactile_features)
        self.tactile_encoders = nn.ModuleList(
            [
                TactileTokenEncoder(
                ...
                for feature in config.tactile_features.values()
            ]
        )
    ...
    def embed_prefix(
        ...
        pad_masks = []
        att_masks = []
        tactile_maps = tactile_maps or []
        tactile_masks = tactile_masks or []
        ...
        # Process tactile maps as prefix tokens so PI0 can condition on touch.
        for encoder, tactile, tactile_mask in zip(
            self.tactile_encoders, tactile_maps, tactile_masks, strict=True
        ):

            def tactile_embed_func(tactile):
                return encoder(tactile)

            tactile_emb = self._apply_checkpoint(tactile_embed_func, tactile)
    ...
    def forward(
        ...
        prefix_embs, prefix_pad_masks, prefix_att_masks = self.embed_prefix(
            images, img_masks, lang_tokens, lang_masks, tactile_maps, tactile_masks
        )
...
class PI0Policy(PreTrainedPolicy):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        try:
            ...
            # Tactile encoders are fork-local heads. They will be randomly initialized when adapting
            # an upstream PI0 checkpoint to a tactile dataset, so do not make them block loading.
            load_strict = strict and not bool(config.tactile_features)
    ...
    def _preprocess_tactile(self, batch: dict[str, Tensor]) -> tuple[list[Tensor], list[Tensor]]:
        """Prepare tactile maps for the tactile token encoders."""
        tactile_features = self.config.tactile_features
        if not tactile_features:
            return [], []

        device = next(self.parameters()).device
        batch_size = next(value.shape[0] for value in batch.values() if isinstance(value, Tensor))
        tactile_maps = []
        tactile_masks = []

        for key, feature in tactile_features.items():
            if key in batch:
                tactile = batch[key].to(device=device, dtype=torch.float32)
                mask = torch.ones(tactile.shape[0], dtype=torch.bool, device=device)
            else:
                tactile = torch.zeros((batch_size, *feature.shape), dtype=torch.float32, device=device)
        ...
        return tactile_maps, tactile_masks
    ...
    def predict_action_chunk(self, batch: dict[str, Tensor], **kwargs: Unpack[ActionSelectKwargs]) -> Tensor:
        ...
        tactile_maps, tactile_masks = self._preprocess_tactile(batch)
    ...
    def _get_default_peft_targets(self) -> dict[str, any]:
        ...
        if self.config.tactile_features:
            target_modules = rf"({target_modules}|model\.tactile_encoders\..*)"
            modules_to_save.append("model.tactile_encoders")
        return {
